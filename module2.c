...
int main(int argc, char **argv) {
  ...
  if (rank == ROOT) {
    ...
    while ((numread = getline(&line, &len, fp)) != -1) {
      ...
      // citations are being counted, and their global ID is stored in listA
      if (checkCitations == 1 && line[0] != '+') {
        ...
        // int returnedIndex = 1; //   NEEDS TO BE DELETED 
      ...
      // indicates that the next line will be the list of cited papers if there
      // are any
      if (line[length - 2] == '-' && line[length - 1] == '\n') {
        // citedCount++;
        // listA[curPaperCount].length++;
        // // printf("ENTER \n");
 ...
 // scatter the lengths matrix 
  ...
  if (rank == 0) {
    ...
    for (int i = 0; i < TOTALPAPERS; i++) {
      ...
      // printf(" rank =%d count =%d  i=%d destRoot =%d cnts[sendRoot]=%d arr[firstelem]= %d localLenghts[i]=%d  \n",rank,count ,  i , sendRoot, length_counts.cnts[sendRoot], listA[i].data[0], listA[i].length);
      count++;
  ...
  // puts("=======");
    ...
    for (  int  i = 0; i < length_counts.cnts[rank]; i++) {
      ...
      MPI_Recv(locallistA[i].data, localLenghts[i] , MPI_INT , ROOT , MPI_ANY_TAG, world , &status );
      if(locallistA[i].data != NULL){
        printf("rank =%d i=%d locallistA[i].data =%d localLenghts[i]=%d %p \n ", rank, i , locallistA[i].data[0], localLenghts[i], locallistA[i].data);

      }else {
        printf("rank =%d i=%d EMPTY locallistA[i].data =%p \n ", rank, i , locallistA[i].data);
